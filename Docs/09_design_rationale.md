...
- **Guided teach protocol.** In-mix signature capture with baseline subtraction measurably degraded models, so teaching an unknown device now walks the user through the same isolate-record protocol as a manual clean recording (OFF baseline, isolated ON capture, OFF tail), auto-advancing on measured power. Design lesson worth reporting: for training data quality, guided isolation beats clever in-situ subtraction.
